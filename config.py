...
# ── Deux environnements (cf. cahier des charges) :
#    - "test"       : ports hauts, ne necessite pas root (developpement local)
#    - "production" : ports reels (22/80/21/25), capture maximale sur VPS expose
#    Bascule via la variable d'environnement HONEYPOT_ENV.
#    Exemple : HONEYPOT_ENV=production sudo -E python3 main.py
ENV = os.environ.get("HONEYPOT_ENV", "test").lower()

if ENV == "production":
    # ── Ports REELS : ce que les bots scannent reellement sur Internet.
    #    Necessite les privileges root (ports < 1024).
    #    Le SSH d'administration doit avoir ete deplace AVANT (cf. harden_vps.sh).
    PORTS = {
        "ssh":    22,
        "telnet": 23,
        "http":   80,
    ...
    # ── Ports de TEST : pas besoin de root, pour le developpement local.
    PORTS = {
        "ssh":    2222,
        "telnet": 2323,
        "http":   8080,
...
# ── Credentials "connus" a titre indicatif/documentaire.
#    CHOIX DE CONCEPTION : le honeypot fonctionne en interaction moyenne et
#    accepte TOUTE authentification (cf. ssh_module.check_auth_password), afin
#    de capturer le comportement POST-authentification de l'attaquant, pas
#    seulement la tentative. Cette liste n'est donc PAS utilisee pour filtrer
#    les connexions ; elle documente les couples les plus courants observes
#    dans les attaques par dictionnaire, et sert de base si l'on souhaite
#    activer un mode "seuil de brute-force realiste" (evolution possible).
FAKE_CREDENTIALS = [
    ("admin",    "admin"),
    ("root",     "root"),
    ("admin",    "password"),
